...
public static class TextInjector
{
    ...
    public static bool InjectText(string text, bool autoEnter = false)
    {
        ...
        if (Log.IsEnabled)
        {
            var fgClass = fgWindow == IntPtr.Zero ? string.Empty : GetWindowClassName(fgWindow);
            var targetClass = GetWindowClassName(target);
            Log.Info($"[TextInjector] Paste target selected: fg={FormatHandle(fgWindow)} class={fgClass}, target={FormatHandle(target)} class={targetClass}, textLen={text.Length}, autoEnter={autoEnter}");
        }

        // Small delay to let the clipboard settle
        Thread.Sleep(ClipboardSettleDelayMs);

        var ctrlVTick = DateTime.UtcNow;
        SendCtrlV();
        Log.Info($"[TextInjector] Sent Ctrl+V for paste after {DateTime.UtcNow - ctrlVTick:g}");
