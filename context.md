...
- New request (2026-02-15): fix Settings ComboBox (dropdown) rendering issues (likely background/blank-area/arrow/border inconsistencies after dark-mode theming changes).
- New request (2026-02-16): fix Settings ComboBox dropdowns immediately collapsing (expand then contract right away when clicked).
- Implemented (2026-02-15): Settings now themes ComboBox dropdown list windows more robustly in dark mode by coloring the listbox background via `WM_CTLCOLORLISTBOX` (so the blank area below items is no longer bright/incorrect) and slightly increasing selection highlight contrast for readability.
...
- Implemented (2026-02-15): ComboBox dropdown items are now owner-drawn with the active theme for dark-mode-friendly item rendering.
- Implemented (2026-02-16): fixed Settings ComboBox dropdowns collapsing immediately by removing native dropdown re-theming on open (`SetWindowTheme`/`DwmSetWindowAttribute`); dark-mode friendliness is handled via owner-draw + `WM_CTLCOLORLISTBOX` listbox background coloring instead.
