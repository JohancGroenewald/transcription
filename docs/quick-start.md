...
Preferred one-command bootstrap for Alpha 1 scaffolds:

```powershell
.\VoiceType2\alpha-build-1\scripts\run-alpha1-all.ps1 -Configuration Debug
```

Run the Alpha 1 test strategy (unit tests + API smoke checks):
...
```powershell
dotnet run --project VoiceType2/alpha-build-1/src/VoiceType2.App.Cli/VoiceType2.App.Cli.csproj -- run --api-url "http://127.0.0.1:5240" --mode managed
```
